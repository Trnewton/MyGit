...
main:	stp	x29, x30, [sp, main_alloc]!		//
	...
	bl	newBox					// Call newBox subroutine
	str	w0, [FP, 16 + b_origin + x]		// Store, first.origin.x = w0 
	str	w1, [FP, 16 + b_origin + y]		// Store, first.origin.y = w1
	str 	w2, [FP, 16 + b_size + width]		// Store, first.size.width = w2
	str	w3, [FP, 16 + b_size + height]		// Store, first.size.height = w3
	str	w4, [FP, 16 + b_area]			// Store, first.area = w4

	bl	newBox					// Call newBox subroutine
	str	w0, [FP, snd_s + b_origin + x]		// Store, second.origin.x = w0
	str	w1, [FP, snd_s + b_origin + y]		// Store, second.origin.y = w1
	str 	w2, [FP, snd_s + b_size + width]	// Store, second.size.width = w2
	str	w3, [FP, snd_s + b_size + height]	// Store, second.size.height = w3
	str	w4, [FP, snd_s + b_area]		// Store, second.area = w4

	adrp	x0, pint				// Load print string
	add	x0, x0, :lo12:pint			// Add low 12 of string
	bl	printf					// Print string
	
	ldr	x0, =first				// Load first string into x0
	add	x1, FP, fst_s				// Add first pointer into x1
	bl	prtBox					// Call printBox subroutine

	ldr	x0, =second
	add	x1, FP, snd_s				// Add second pointer into x1
	bl	prtBox					// Call printBox subroutine

	add	x0, FP, fst_s				// Add first pointer into x0
	add	x1, FP, snd_s				// Add second pointer into x1
	bl	equal					// Call subroutine equal
	cbz	x0, noteq				// If boxs are not equal branch
	
	add	x0, FP, fst_s				// Add first pointer into x0
	mov	w1, #-5					// Set w1 = deltax = -5
	mov	w2, #7					// Set w2 - deltayt = 7
	bl	move					// Call move subroutine
	
	add	x0, FP, snd_s				// Add second pointer into x0
	mov	w1, #3					// Set w1 = factor = 3
	bl	expand					// Call expand subroutine

noteq:	adrp	x0, pchg				// Load print string into x0
	add	x0, x0, :lo12:pchg			// Add low 12 into x0
	bl	printf					// Print string

	adrp	x0, first				// Load first string into x0
	add	x0, x0, :lo12:first			// Add low 12 into x0
	add	x1, FP, fst_s				// Add &first into x1
	bl	prtBox					// Call printBox subroutine

	adrp	x0, second				// Load second string into x0
	add	x0, x0, :lo12:second			// Add low 12 to x0
	add	x1, FP, snd_s				// Add &second into x1
	bl	prtBox					// Call printBoc subroutine
